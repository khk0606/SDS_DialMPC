...
class DialSim:
    def __init__(
        ...
        # safety clamp (hard stop for jumping)
        self.startup_hold_sec = 1.0
        self.z_max = 0.60
        self.vz_max = 2.5
        self.safe_ctrl = self.default_q[7:7+self.Nu].copy()


    def _in_startup_hold(self):
        return self.t < self.startup_hold_sec

    def _unsafe_state(self):
        z = float(self.mj_data.qpos[2])
        vz = float(self.mj_data.qvel[2])
        return (z > self.z_max) or (vz > self.vz_max)

    def _apply_safety(self, ctrl, tau=None):
        unsafe = self._unsafe_state()
        if self._in_startup_hold() or unsafe:
            if unsafe:
                if self.mj_data.qpos[2] > self.z_max:
                    self.mj_data.qpos[2] = self.z_max
                if self.mj_data.qvel[2] > 0.0:
                    self.mj_data.qvel[2] = 0.0
                import mujoco
                mujoco.mj_forward(self.mj_model, self.mj_data)
            if tau is not None:
                import numpy as np
                tau = np.zeros_like(tau)
            return self.safe_ctrl, tau
        return ctrl, tau
    ...
    def main_loop(self):
        ...
        # DISABLE_REF_GEOMS: ref trajectory visualization off
        ...
        while True:
            if self.plot:
                ...
                plt.pause(0.001)
            # ref geom update disabled
            if self.sync_mode:
                while self.t <= (self.plan_time_shared[0] + self.ctrl_dt):
                    if self.leg_control == "position":
                        ctrl, _ = self._apply_safety(self.acts_shared[0])
                        self.mj_data.ctrl = ctrl
                    elif self.leg_control == "torque":
                        _, tau = self._apply_safety(self.acts_shared[0], self.tau_shared[0])
                ...
                if self.leg_control == "position":
                    ctrl, _ = self._apply_safety(self.acts_shared[delta_step])
                    self.mj_data.ctrl = ctrl
                elif self.leg_control == "torque":
                    _, tau = self._apply_safety(self.acts_shared[delta_step], self.tau_shared[delta_step])
                    self.mj_data.ctrl = tau
                if self.record:
                    self.data.append(
                        np.concatenate(
