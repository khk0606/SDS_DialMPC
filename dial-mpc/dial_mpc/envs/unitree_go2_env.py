...
from dial_mpc.envs.sds_reward_function import compute_sds_reward
...
class UnitreeGo2EnvConfig(BaseEnvConfig):
    ...
    fixed_vel_tar: Union[Sequence[float], None] = None
    fixed_ang_vel_tar: Union[Sequence[float], None] = None
...
class UnitreeGo2Env(BaseEnv):
    ...
    def reset(self, rng: jax.Array) -> State:  # pytype: disable=signature-mismatch
        ...
        if self._config.fixed_vel_tar is not None:
            state_info["vel_tar"] = jnp.array(self._config.fixed_vel_tar)
        if self._config.fixed_ang_vel_tar is not None:
            state_info["ang_vel_tar"] = jnp.array(self._config.fixed_ang_vel_tar)
    ...
    def step(self, state: State, action: jax.Array) -> State:
        ...
        if self._config.fixed_vel_tar is not None:
            fixed = jnp.array(self._config.fixed_vel_tar)
            t_ramp = jnp.maximum(0.0, t - self._config.startup_hold_sec)
            ramp = jnp.minimum(t_ramp / self._config.ramp_up_time, 1.0)
            state.info["vel_tar"] = fixed * ramp
            # MAX_VEL_RAMP: hard cap on early target speed
            max_v = jnp.minimum(0.6, 0.2 + 0.2 * (t_ramp))
            state.info["vel_tar"] = jnp.array([jnp.minimum(state.info["vel_tar"][0], max_v), 0.0, 0.0])
        else:
            state.info["vel_tar"] = jnp.minimum(
                vel_tar * t / self._config.ramp_up_time, vel_tar
            )

        if self._config.fixed_ang_vel_tar is not None:
            fixed_ang = jnp.array(self._config.fixed_ang_vel_tar)
            t_ramp = jnp.maximum(0.0, t - self._config.startup_hold_sec)
            ramp = jnp.minimum(t_ramp / self._config.ramp_up_time, 1.0)
            state.info["ang_vel_tar"] = fixed_ang * ramp
        else:
...
# reward
        ...
        # gait target (MUST KEEP for state.info["z_feet_tar"])
        ...
        # SDS reward calculation
        reward = compute_sds_reward(pipeline_state, state.info, self)
